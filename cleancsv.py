...
def get_reader_writer(file, rw: str):
    ...
    try:
        if rw == "r":
            with open(
                file, "r", newline="", encoding="utf-8", errors="replace"
            ) as f:
                reader = csv.reader(
                    f,
                    delimiter="\t",
            ...
            with open(
                file, "a", newline="", encoding="utf-8", errors="replace"
            ) as f:
                writer = csv.writer(
                    f,
                    delimiter="\t",
...
class CleanCsv:
    def __init__(self, csvfile) -> None:
        ...
        self.js_name = (
            f"{JSON_DIR}/table-dtypes/{self.name.replace('.csv', '.json')}"
        )
        self.no_nul = os.path.abspath(self.csvfile).replace(
            ".csv", "_no_nul.csv"
        )
    ...
    def copy_to_table(self, connection, postfix, table="") -> None:
        ...
        with open_db(connection) as curs:
            ...
            # iter_csv = StringIteratorIO(iter(self.csv_gen_pk()))
            # curs.copy_from(iter_csv, table, sep="|", null="\\N", size=8192)
            copy_statement = f"COPY {table} FROM STDIN WITH (FORMAT TEXT, DELIMITER '|', NULL '\\N')"
                    ...
                    if not row:
                        ...
                        if _bad_vals:
                            ...
                            if not copy or not self.get_bad_values(copy):
                                yield self.clean_row(
                                    self.remove_extra_cols(copy)
                                )
                        elif not _bad_vals:
        ...
        for i, value in enumerate(row):
            try:
                ...
                if self.is_nul_like(value):
                    ...
                    if (
                        value not in codes[dtype].keys()
                    ):  # see if value is in lookups
                        bad_values.append((i, value))
                elif dtype == "timestamp without time zone":
    ...
    def get_codes(self) -> dict:
        ...
        """
        json_files = [
            file for file in self.dtypes.values() if file.endswith(".json")
        ]
        json_dicts = []
